...
void *periodic_polling_thread(void *args)
{
    int pinIn  = *(int *)args;
    ...
    for (int i = 0; i < TEST_COUNT; i++)
    {
        ...
        sleep_realtime(POLL_PERIOD_MS * 1e6);
    }
...
void *disturbance_thread(void *args)
{
    while (1)
    {
        asm volatile("" ::: "memory"); // Dummy operation to prevent optimization
    }
}
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&threadA, NULL, periodic_polling_thread, &pinInA);
    set_cpu(cpu_id);

    pthread_create(&threadB, NULL, periodic_polling_thread, &pinInB);
    set_cpu(cpu_id);

    pthread_create(&threadC, NULL, periodic_polling_thread, &pinInC);
    set_cpu(cpu_id);
