...
int main (void){
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        if (gpio_get_pin_value(TEST_A) == 0)
        {
            ...
            while (gpio_get_pin_value(TEST_A) == 0){}; // wait for TEST_A to go high
        ...
        if (gpio_get_pin_value(TEST_B) == 0)
        {
            ...
            while (gpio_get_pin_value(TEST_B) == 0){}; // wait for TEST_B to go high
        ...
        if (gpio_get_pin_value(TEST_C) == 0)
        {
            ...
            while (gpio_get_pin_value(TEST_C) == 0){}; // wait for TEST_C to go high
            gpio_set_pin_high(RESPONSE_C);
            print_debug("RESPONSE_C set to high\n", debug);
        }

        busy_delay_ms(500); // prevent rapid toggling
    }
