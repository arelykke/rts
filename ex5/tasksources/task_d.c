...
void print_debug(char* message, bool debug) {
    if (debug) {
        printf(message);
    }
}
...
__attribute__((__interrupt__)) static void interrupt_J3(void) {
    // Check which pin triggered the interrupt and respond accordingly
    ...
    if (gpio_get_pin_interrupt_flag(TEST_B)) {
        ...
        busy_delay_us(100); // Simulating an expensive operation
...
// Deferred handling without delay for B
void handle_events_deferred_no_delay() {
    while (1) {
...
// Deferred handling with delay for B
void handle_events_deferred_with_delay() {
    while (1) {
        if (gpio_get_pin_value(TEST_A)) {
            gpio_set_pin_low(RESPONSE_A);
            print_debug("RESPONSE_A set to low\n", debug);
...
int main(void) {
    ...
    // Uncomment one of the following to test the desired response method:
    
    // Use interrupt handling
    // while (1) {
    //     // Main loop can do other tasks or just idle
    // }

    // Use deferred handling without delay
    // handle_events_deferred_no_delay();

    // Use deferred handling with delay
    // handle_events_deferred_with_delay();

    while (1) {
        gpio_toggle_pin(LED0_GPIO);
        print_debug("tick\n", debug);
        busy_delay_us(500); // Example delay for main loop
    }
